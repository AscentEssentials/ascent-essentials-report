...
Mongoose\cite{mongoose} è una libreria JavaScript progettata per semplificare l'interazione con MongoDB, il database NoSQL orientato ai documenti. Questa libreria offre un'interfaccia intuitiva e potente per gestire le operazioni di CRUD (Create, Read, Update, Delete) nei database MongoDB. 
...
Socket.io\cite{socketio} è una libreria JavaScript che consente la comunicazione in tempo reale tra client e server attraverso il protocollo WebSocket. Con una sintassi semplice e potente, Socket.io semplifica la gestione degli eventi bidirezionali, consentendo agli sviluppatori di creare applicazioni web interattive e dinamiche.

\section{JSON Web Tokens}

JSON Web Token (JWT)\cite{jwt} è uno standard aperto (RFC 7519) che definisce un formato compatto e autosufficiente per la rappresentazione sicura di informazioni tra due parti. La libreria è uno strumento essenziale per l'implementazione e la gestione di token nei progetti software. 
